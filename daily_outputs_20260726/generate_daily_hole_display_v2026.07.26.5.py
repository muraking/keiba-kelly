...
VERSION = "v2026.07.26.5"
_CIRCLED = {number: chr(0x2460 + number - 1) for number in range(1, 21)}


def circled(number) -> str:
    try:
        value = int(number)
...
def fmt_horse(prefix: str, number: str, snap: dict, pp=None, buy=None) -> str:
    ...
    if buy:
        suffix += "　●買い " + "/".join(buy)
    return f"{prefix} {circled(number)} {name}　WP{wp:>4.1f}%　{odds:>5.1f}倍{suffix}"
    ...
    for market, races in markets.items():
        ...
        for row in races:
            ...
            for rank, number in enumerate(row["top2"], 1):
                lines.append(fmt_horse(f"勝{circled(rank)}", number, snap))
            lines += ["", "【穴馬候補・暫定WP順】"]
            if row["holes"]:
                for rank, number in enumerate(row["holes"], 1):
                    lines.append(fmt_horse(
                        f"穴{circled(rank)}", number, snap, row["pp"][number], row["buys"].get(number)
                    ))
            else:
                lines.append("穴候補なし")
            lines.append("")
            if row["tickets"]:
                lines.append(
                    "【当時の既存判定買い目】 "
                    + "／".join(circled_ticket(ticket) for ticket in row["tickets"])
                )
                if market == "local":
...
def main():
    ...
    render(results, a.dir / "all_races_display_v2026.07.26.5.md")
    (a.dir / "all_races_display_v2026.07.26.5.json").write_text(
