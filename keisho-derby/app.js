...
function studioLogoMarkup(){return '<span class="studio-company-mark"><span>Studio</span><strong class="studio-ba"><b>B</b><i>☆</i><b>A</b></strong><span>Co., Ltd.</span></span>'}
document.querySelectorAll(".ending-credit").forEach(element=>element.remove());
if(document.querySelector("#endingCompanyCard"))document.querySelector("#endingCompanyCard").innerHTML=studioLogoMarkup();
const coatPalettes={
...
function weightedPick(items,weights){let roll=Math.random()*weights.reduce((a,b)=>a+b,0);for(let i=0;i<items.length;i++){roll-=weights[i];if(roll<=0)return items[i]}return items[items.length-1]}
function coatVariant(coatName){const palette=coatPalettes[coatName]||coatPalettes["栗毛"];return palette[rnd(0,palette.length-1)]}
function chooseCoat(parentAppearance=null){
  const parentCoat=parentAppearance?.coat;
  const whiteChance=parentCoat==="白毛"?.08:.0015;
  if(Math.random()<whiteChance)return["白毛",coatVariant("白毛")];
  if(parentCoat&&parentCoat!=="白毛"&&Math.random()<.58)return[parentCoat,coatVariant(parentCoat)];
  const name=weightedPick(["栗毛","栃栗毛","鹿毛","黒鹿毛","青鹿毛","青毛","芦毛"],[25,7,31,13,7,3,14]);
  return[name,coatVariant(name)];
}
function createAppearanceDNA(coat,parent=null){
  const inherit=key=>parent&&parent[key]!==undefined&&Math.random()<.48?parent[key]:null;
  const faceMarkType=inherit("faceMarkType")||weightedPick(FACE_MARK_TYPES,[55,13,13,4,6,6,3]);
  const parentLegs=Array.isArray(parent?.legMarks)?parent.legMarks:[];
  const legMarks=Array.from({length:4},(_,i)=>Math.random()<.52&&Number.isFinite(parentLegs[i])?parentLegs[i]:weightedPick([0,1,2],[72,20,8]));
  return{coat:coat[0],color:coat[1],shade:rnd(0,4),faceMarkType,legMarks,
    maneStyle:inherit("maneStyle")||MANE_STYLES[rnd(0,MANE_STYLES.length-1)],
    tailStyle:inherit("tailStyle")||TAIL_STYLES[rnd(0,TAIL_STYLES.length-1)]};
}
function normalizeAppearanceDNA(candidate){
  ...
  candidate.coat=candidate.appearanceDNA.coat;candidate.color=candidate.appearanceDNA.color;
  candidate.faceMark=candidate.appearanceDNA.faceMarkType!=="none";candidate.socks=candidate.appearanceDNA.legMarks.filter(Boolean).length;
  return candidate.appearanceDNA;
}
function appearanceClassNames(appearance){
  const a=appearance||{},legs=Array.isArray(a.legMarks)?a.legMarks:[0,0,0,0];
  return[`face-${a.faceMarkType||"none"}`,`mane-${a.maneStyle||"standard"}`,`tail-${a.tailStyle||"standard"}`,
    `front-sock-${legs[0]||0}`,`back-sock-${legs[2]||legs[1]||0}`].join(" ");
}
function applyAppearanceToElement(el,candidate){
  if(!el||!candidate)return;const a=normalizeAppearanceDNA(candidate),known=[...FACE_MARK_TYPES.map(x=>`face-${x}`),...MANE_STYLES.map(x=>`mane-${x}`),...TAIL_STYLES.map(x=>`tail-${x}`),"front-sock-0","front-sock-1","front-sock-2","back-sock-0","back-sock-1","back-sock-2","has-face-mark","has-socks"];
  el.classList.remove(...known);el.classList.add(...appearanceClassNames(a).split(" "));el.style.setProperty("--active-horse-color",a.color);el.style.setProperty("--horse-color",a.color);el.querySelectorAll(".candidate-pixel-horse").forEach(h=>h.style.setProperty("--horse-color",a.color));
}
const sires = [
...
let detailHorseMotionTimer=null,detailHorseMotionIndex=0;
...
async function toggleTitleBgm(){
  if(!titleBgm)return;
  audioPrefs.bgm=!audioPrefs.bgm;saveAudioPrefs();renderAudioSettings();syncBgm();
}
...
function loadGame(slot=activeSaveSlot){
  try{
    ...
    if(typeof saved?.equippedTackColor!=="string")game.equippedTackColor="#5aa8df";
    ...
    if(game.candidate)normalizeAppearanceDNA(game.candidate);
...
function generateCandidate(){
  const coat=chooseCoat(),appearanceDNA=createAppearanceDNA(coat),sire=sires[rnd(0,sires.length-1)], dam=dams[rnd(0,dams.length-1)];
  ...
  const candidate={coat:coat[0],color:coat[1],appearanceDNA,sex:Math.random()<.5?"牡馬":"牝馬",faceMark:appearanceDNA.faceMarkType!=="none",socks:appearanceDNA.legMarks.filter(Boolean).length,eyeType:["優しい","鋭い","好奇心旺盛"][rnd(0,2)],sire,dam,speed:rnd(390,560),dash:rnd(370,540),gateSkill:rnd(380,540),stamina:rnd(390,560),power:rnd(380,550),guts:rnd(370,540),turf,dirt,heavyTrack:Math.max(350,Math.min(800,rnd(430,670)+parentHeavyBase*10)),temperamentValue,temperament:temperamentType(temperamentValue),baseBestWeight,weight:baseBestWeight+rnd(12,20),growthType,growthPotential:rnd(7,15),distanceMin,distanceMax,soundness:rnd(380,780),recoveryPower:rnd(380,760),turnaroundTolerance:rnd(340,740),...createConditionCycle()};
  ...
  applyAppearanceToElement(document.querySelector("#candidateHorse"),candidate);
...
function beginNextGeneration(partner){
  ...
  const [distanceMin,distanceMax]=createDistanceRange(sire,dam);
  const parentAppearance=normalizeAppearanceDNA(game.candidate),coat=chooseCoat(parentAppearance),appearanceDNA=createAppearanceDNA(coat,parentAppearance);
  const level=partner.level;
  const child={
    coat:coat[0],color:coat[1],appearanceDNA,sex:Math.random()<.5?"牡馬":"牝馬",faceMark:appearanceDNA.faceMarkType!=="none",socks:appearanceDNA.legMarks.filter(Boolean).length,eyeType:["優しい","鋭い","好奇心旺盛"][rnd(0,2)],sire,dam,
  ...
  applyAppearanceToElement(document.querySelector("#candidateHorse"),child);
  ...
  applyAppearanceToElement(document.querySelector("#birthHorse"),child);
...
const TACK_COLORS=[
  {name:"青",value:"#5aa8df"},{name:"赤",value:"#e94e45"},{name:"黄",value:"#f0c84b"},{name:"緑",value:"#51a85b"},
  {name:"白",value:"#f2ead7"},{name:"黒",value:"#25282b"},{name:"桃",value:"#ed82a7"},{name:"紫",value:"#8759b7"},{name:"橙",value:"#ed8a3c"},{name:"水色",value:"#69cfda"}
];
...
function renderDetailTackPanel(){
  renderTack();
  const colors=document.querySelector("#tackColorChoices");
  colors.innerHTML=TACK_COLORS.map(color=>`<button data-tack-color="${color.value}" class="${game.equippedTackColor===color.value?"selected":""}" style="--choice-color:${color.value}" title="${color.name}"><i></i><span>${color.name}</span></button>`).join("");
  colors.querySelectorAll("button").forEach(button=>button.disabled=!game.equippedTack);
  document.querySelector("#tackColorGuide").textContent=game.equippedTack?`${tackCatalog[game.equippedTack].name}・${TACK_COLORS.find(c=>c.value===game.equippedTackColor)?.name||"青"}を装着中です。`:game.tackUnlocked.length?"馬具を選ぶと色を変更できます。":"調教やレースを重ねると、調教師から馬具が提案されます。";
}
function applyHorseAppearance(el){
  if(!el)return;
  applyAppearanceToElement(el,game.candidate);
  el.style.setProperty("--tack-color",game.equippedTackColor||"#5aa8df");
...
function startDetailHorseMotion(){
  if(detailHorseMotionTimer)clearInterval(detailHorseMotionTimer);
  const stage=document.querySelector("#detailHorseStage"),horse=stage?.querySelector(".detail-pixel-horse");if(!horse)return;
  const motions=[{className:"detail-motion-idle",text:"こちらの様子をじっと見ています。"},{className:"detail-motion-approach",text:"興味深そうに近寄ってきました。"},{className:"detail-motion-hop",text:"元気に小さく跳ねています。"},{className:"detail-motion-rest",text:"安心したのか、ゆっくり伏せました。"},{className:"detail-motion-wake",text:"耳を動かしながら起き上がりました。"}];
  const next=()=>{horse.classList.remove(...motions.map(m=>m.className),"horse-happy");const motion=motions[detailHorseMotionIndex++%motions.length];horse.classList.add(motion.className);const reaction=document.querySelector("#horseReaction");if(reaction)reaction.textContent=motion.text};
  next();detailHorseMotionTimer=setInterval(next,4700);
}
function renderHorseDetail(){
  const stage=document.querySelector("#detailHorseStage");
  applyHorseAppearance(stage);
  startDetailHorseMotion();
  ...
  document.querySelector("#horseDetailInfo").innerHTML=`<h2>${game.horseName}</h2><dl>
    <dt>性別・毛色</dt><dd>${game.candidate?.sex||"牡馬"}・${game.candidate?.coat||"栗毛"}・${({none:"顔白なし",star:"星",blaze:"流星",doubleBlaze:"二本流星",snip:"鼻白",starSnip:"星・鼻白",bald:"大流星"})[game.candidate?.appearanceDNA?.faceMarkType||"none"]}</dd>
    <dt>外見</dt><dd>たてがみ ${({short:"短め",standard:"標準",long:"長め",upright:"立ち",wavy:"波打ち"})[game.candidate?.appearanceDNA?.maneStyle||"standard"]}・尻尾 ${({short:"短め",standard:"標準",long:"長め",wavy:"波打ち",raised:"上向き"})[game.candidate?.appearanceDNA?.tailStyle||"standard"]}</dd>
...
const galleryCatalog=[
  ...
  {id:"gen7",title:"七代の蹄跡",trophy:"gold",condition:g=>g.generation>=7,desc:"7代にわたり血統を継承"},
  {id:"gen15",title:"血統年代記",trophy:"record",condition:g=>g.generation>=15,desc:"15代続く愛馬の歴史"},
  {id:"gen20",title:"二十代王朝",trophy:"grand",condition:g=>g.generation>=20,desc:"20代の大血統を築いた"},
  {id:"gen30",title:"終わらない物語",trophy:"record",condition:g=>g.generation>=30,desc:"30代にわたり夢を継承"},
  {id:"gen50",title:"永遠の血脈",trophy:"record",condition:g=>g.generation>=50,desc:"50代へ到達した伝説の系譜"},
  {id:"lifeWin10",title:"一族十勝",trophy:"silver",condition:()=>lifetimeTotal("wins")>=10,desc:"血統通算10勝"},
  {id:"lifeWin25",title:"勝利を継ぐ者",trophy:"gold",condition:()=>lifetimeTotal("wins")>=25,desc:"血統通算25勝"},
  {id:"lifeWin50",title:"五十の歓声",trophy:"grand",condition:()=>lifetimeTotal("wins")>=50,desc:"血統通算50勝"},
  {id:"lifeWin100",title:"百勝一族",trophy:"record",condition:()=>lifetimeTotal("wins")>=100,desc:"血統通算100勝"},
  {id:"lifeRace100",title:"百戦の系譜",trophy:"gold",condition:()=>lifetimeTotal("races")>=100,desc:"血統通算100戦"},
  {id:"lifeRace250",title:"旅を続ける血統",trophy:"grand",condition:()=>lifetimeTotal("races")>=250,desc:"血統通算250戦"},
  {id:"lifeRace500",title:"五百の蹄音",trophy:"record",condition:()=>lifetimeTotal("races")>=500,desc:"血統通算500戦"},
  {id:"lifePrize10",title:"一族の宝庫",trophy:"gold",condition:()=>lifetimeTotal("prize")>=100000,desc:"血統通算賞金10億円"},
  {id:"lifePrize30",title:"黄金の系譜",trophy:"grand",condition:()=>lifetimeTotal("prize")>=300000,desc:"血統通算賞金30億円"},
  {id:"lifePrize100",title:"競馬史の財宝",trophy:"record",condition:()=>lifetimeTotal("prize")>=1000000,desc:"血統通算賞金100億円"},
  {id:"graded5",title:"重賞ハンター",trophy:"silver",condition:g=>(g.gradedTrophies||[]).length>=5,desc:"異なる重賞を5勝"},
  {id:"graded15",title:"重賞コレクター",trophy:"gold",condition:g=>(g.gradedTrophies||[]).length>=15,desc:"異なる重賞を15勝"},
  {id:"graded30",title:"全国行脚",trophy:"grand",condition:g=>(g.gradedTrophies||[]).length>=30,desc:"異なる重賞を30勝"},
  {id:"graded60",title:"トロフィーホール",trophy:"record",condition:g=>(g.gradedTrophies||[]).length>=60,desc:"異なる重賞を60勝"},
  {id:"g1three",title:"GⅠの主役",trophy:"gold",condition:g=>(g.gradedTrophies||[]).filter(t=>t.grade==="G1").length>=3,desc:"異なるGⅠを3勝"},
  {id:"g1ten",title:"十冠の血統",trophy:"grand",condition:g=>(g.gradedTrophies||[]).filter(t=>t.grade==="G1").length>=10,desc:"異なるGⅠを10勝"},
  {id:"overseas3",title:"世界への足跡",trophy:"grand",condition:()=>lifetimeRaces().filter(r=>r.place===1&&OVERSEAS_G1.some(o=>String(r.raceName||"").startsWith(o.name))).length>=3,desc:"海外GⅠを3勝"},
  {id:"turfDirtLegacy",title:"二つの王道",trophy:"record",condition:()=>lifetimeRaces().some(r=>r.place===1&&String(r.course).includes("芝"))&&lifetimeRaces().some(r=>r.place===1&&String(r.course).includes("ダート")),desc:"血統で芝・ダート双方を制覇"},
  {id:"whiteLegacy",title:"白き奇跡",trophy:"record",condition:g=>g.candidate?.coat==="白毛"||(g.retirementRecords||[]).some(h=>h.coat==="白毛"),desc:"白毛馬が血統に誕生"}
];
...
let endingPreviewMode=false,endingTimers=[],activeEndingId=null;
const ENDING_MOCK_HORSES=["ドットスター","ピクセルエース","ミントウイング","グランドエース","チェリーミスト","ムーンウイング","サンライズステップ","ノーブルソング"];
...
function endingEntries(id,preview){
  return endingTargets(id).map((raceName,index)=>{
    const won=(game.gradedTrophies||[]).find(t=>t.raceName===raceName);
    return won||{raceName,grade:raceCalendar.find(r=>r.name===raceName)?.raceClass||"G3",horseName:preview?ENDING_MOCK_HORSES[index%ENDING_MOCK_HORSES.length]:"歴代の愛馬",generation:index%6+1,horseColor:["#9b542e","#6f3f2b","#b46a35","#55423a","#d0c4aa"][index%5]};
  });
}
...
function playOverseasEnding(entries){
  ...
  const next=()=>{if(index>=entries.length)return endingFinale(activeEndingId);const entry=entries[index++],order=entry.finishSnapshot?.order||[],pack=(order.length?order.slice(0,4):[{player:true,color:entry.horseColor,id:1},{color:"#75452f",id:2},{color:"#b26a36",id:3}]);cinematic.innerHTML=`<div class="ending-overseas-finish ${entry.finishSnapshot?.weather||""}"><div class="ending-finish-post"><i></i><b>GOAL</b></div><div class="ending-finish-pack">${pack.map((runner,i)=>{const number=Math.max(1,Math.min(8,Number(runner.id)||i+1));return `<div class="ending-finish-runner" style="--finish-gap:${i*16}px">${endingHorseMarkup(runner.player?entry:{horseColor:runner.horseColor||runner.color},i)}<em class="frame-${number}">${number}</em></div>`}).join("")}</div><article><small>WORLD GⅠ ${index} / ${entries.length}</small><h2>${entry.raceName}</h2><b>${entry.horseName}</b><span>${entry.finishSnapshot?.time||""}</span></article></div>`;endingLater(next,3000)};next();
}
function playAllGradedEnding(entries,preview=false){
  const cinematic=document.querySelector("#endingCinematic"),minor=entries.filter(e=>e.grade!=="G1");
  const rollDuration=Math.min(70000,Math.max(26000,minor.length*720));
  cinematic.innerHTML=`<div class="ending-text-roll" style="--roll-duration:${rollDuration}ms"><div>${[...minor].reverse().map(e=>`<p><small>${e.grade==="G2"?"GⅡ":"GⅢ"}</small><b>${e.raceName}</b><span>${e.generation||"-"}代目 ${e.horseName}</span></p>`).join("")}</div></div>`;
  endingLater(()=>{
    const lineage=[...(game.retirementRecords||[]),{name:game.horseName,appearanceDNA:game.candidate?.appearanceDNA}].filter(h=>h.name);
    const mock=ENDING_MOCK_HORSES.map((name,i)=>({name,horseColor:["#9b542e","#6f3f2b","#b46a35","#55423a"][i%4]}));
    const horses=(preview||!lineage.length?mock:lineage).slice(-12);
    cinematic.innerHTML=`<div class="ending-parade"><h2>受け継がれた蹄跡</h2><div>${horses.map((horse,i)=>`<article style="--parade-delay:${i*180}ms">${endingHorseMarkup(horse,i)}<b>${horse.name}</b></article>`).join("")}</div></div>`;
    endingLater(()=>endingFinale(activeEndingId),6200);
  },rollDuration);
}
function startEndingCinematic(id,preview){
  clearEndingTimers();activeEndingId=id;const screen=document.querySelector("#endingScreen"),entries=endingEntries(id,preview);
  screen.classList.remove("finale","company-only");screen.classList.add("playing");document.querySelector("#endingCompanyCard").hidden=true;document.querySelector("#endingContinueButton").hidden=true;document.querySelector("#endingSkipButton").hidden=false;
  if(id==="domestic-g1")playDomesticEnding(entries);else if(id==="overseas-g1")playOverseasEnding(entries);else playAllGradedEnding(endingEntries("all-graded",preview),preview);
}
function showEnding(id,preview=false){
  ...
  startEndingCinematic(id,preview);
}
function renderEndingGallery(){
  const container=document.querySelector("#endingGalleryGrid"),section=document.querySelector("#endingGallerySection");if(!container||!section)return;
  const unlockedIds=Object.keys(ENDING_DEFINITIONS).filter(id=>(game.endingUnlocks||[]).includes(id));
  section.hidden=unlockedIds.length===0;
  container.innerHTML=unlockedIds.map(id=>{const item=ENDING_DEFINITIONS[id];return `<button class="ending-gallery-card unlocked" data-ending-replay="${id}"><small>${item.chapter}</small><b>${item.title}</b><span>もう一度見る</span></button>`}).join("");
}
function syncDeveloperEndingButtons(){
  const controls=document.querySelector("#endingDevButtons");if(controls)controls.hidden=!developerMode;
}
...
let selectedTrophyIndex=null,renderedTrophyCatalog=[];
function trophyCatalog(grade){
  const unique=new Map();
  raceCalendar.filter(r=>r.raceClass===grade).sort((a,b)=>(a.week-b.week)||a.name.localeCompare(b.name,"ja")).forEach(r=>{if(!unique.has(r.name))unique.set(r.name,r)});
  return [...unique.values()];
}
function renderTrophies(grade=selectedTrophyGrade){
  selectedTrophyGrade=grade;
  selectedTrophyIndex=null;
  const trophies=game.gradedTrophies||[],winsByRace=new Map();
  trophies.filter(t=>t.grade===grade).forEach(t=>{if(!winsByRace.has(t.raceName))winsByRace.set(t.raceName,t)});
  renderedTrophyCatalog=trophyCatalog(grade).map(r=>({race:r,trophy:winsByRace.get(r.name)||null}));
  const gradeLabel=grade==="G1"?"GⅠ":grade==="G2"?"GⅡ":"GⅢ";
  document.querySelector("#trophySummary").textContent=`${gradeLabel} ${renderedTrophyCatalog.filter(x=>x.trophy).length}／${renderedTrophyCatalog.length}`;
  ...
  document.querySelector("#trophyGrid").innerHTML=renderedTrophyCatalog.map(({race,trophy},index)=>`<button class="trophy-card grade-${grade.toLowerCase()} ${trophy?"won":"locked"}" data-trophy-index="${index}"><span class="trophy-cup-icon">♛</span><b>${race.name}</b><small>${trophy?trophy.date||"獲得済み":"未獲得"}</small>${trophy?`<span class="trophy-winner-name" data-trophy-horse="${index}">${trophy.horseName}<small>この馬を見る</small></span>`:""}</button>`).join("");
  closeTrophyHorse();
}
function showTrophyHorse(index){
  const detail=document.querySelector("#trophyHorseDetail"),modal=document.querySelector("#trophyHorseModal");
...
function closeTrophyHorse(){const modal=document.querySelector("#trophyHorseModal");if(modal){modal.classList.remove("show");modal.setAttribute("aria-hidden","true")}}
function train(type){
...
function prepareRace(race){
  ...
  try{
    ...
    saveGame();
    dispatchEvent(new CustomEvent("dotkeiba:prepare",{detail:{horseName:game.horseName,sex:game.candidate?.sex||"牡馬",appearance:game.candidate?.appearanceDNA||null,sexCondition:race.sexCondition||(raceFemaleOnly(race)?"牝馬限定":"混合"),raceName:race.name,age:horseAge(),ability:playerAbility(race),dash:game.dash,gateSkill:game.gateSkill,condition:game.condition,fatigue:game.fatigue,difficulty:race.difficulty*10,raceClass:race.raceClass,overseas:!!race.overseas,venue:raceVenue(race),distance:race.distance,surface:race.surface,direction:race.direction||null,courseAuditMode:!!race.courseAuditMode,heavyTrack:game.heavyTrack,temperament:game.temperament,temperamentValue:game.temperamentValue,equippedTack:game.equippedTack,equippedTackColor:game.equippedTackColor,weather:raceWeather.weather,going:raceWeather.going,raceMonth:raceWeather.month,baseTime:benchmarkTime,benchmarkTime,recordTime,recordVerified:timingRecord.verified,layoutV2:true}}));
  }catch(error){
    ...
    dispatchEvent(new CustomEvent("dotkeiba:prepare",{detail:{
      horseName:game.horseName,sex:game.candidate?.sex||"牡馬",appearance:game.candidate?.appearanceDNA||null,sexCondition:race.sexCondition||(raceFemaleOnly(race)?"牝馬限定":"混合"),raceName:race.name,age:horseAge(),ability:playerAbility(race),dash:game.dash,gateSkill:game.gateSkill,equippedTack:game.equippedTack,equippedTackColor:game.equippedTackColor,
...
function showResult(detail){
  ...
  if(place===1&&["G1","G2","G3"].includes(r.raceClass)){
    ...
    else if(r.overseas&&!existingTrophy.finishSnapshot)existingTrophy.finishSnapshot=finishSnapshot;
  }
  checkEndingUnlocks();
...
document.querySelector("#endingDevAccessButton").onclick=()=>{
  developerMode=true;localStorage.setItem("dotKeibaDeveloperMode","1");syncDeveloperEndingButtons();
};
...
document.querySelector("#trophyGrid").onclick=e=>{
  const horse=e.target.closest("[data-trophy-horse]");if(horse){e.stopPropagation();showTrophyHorse(Number(horse.dataset.trophyHorse));return}
  const button=e.target.closest("[data-trophy-index]");if(!button||button.classList.contains("locked"))return;
  const index=Number(button.dataset.trophyIndex),same=selectedTrophyIndex===index;selectedTrophyIndex=same?null:index;
  document.querySelectorAll("[data-trophy-index]").forEach(card=>card.classList.toggle("selected",!same&&Number(card.dataset.trophyIndex)===index));
};
...
document.querySelector("#detailTackButton").onclick=()=>{const panel=document.querySelector("#detailTackPanel");panel.hidden=!panel.hidden;if(!panel.hidden)renderDetailTackPanel();document.querySelector("#detailTackButton").textContent=panel.hidden?"馬具を変更する":"馬具メニューを閉じる"};
...
document.querySelector("#returnHomeButton").onclick=()=>{
  const pending=(game.pendingEndings||[])[0];
  if(pending){showEnding(pending);return}
  renderHome(postRaceTrainerComment().replace(/^調教師「|」$/g,""));showScreen("homeScreen");
};
document.querySelector("#endingContinueButton").onclick=()=>{
  clearEndingTimers();
  if(endingPreviewMode){showScreen("titleScreen");return}
  const finished=(game.pendingEndings||[]).shift();
  if(finished&&!game.endingSeen.includes(finished))game.endingSeen.push(finished);
  saveGame();
  const next=(game.pendingEndings||[])[0];if(next){showEnding(next);return}
  renderHome(postRaceTrainerComment().replace(/^調教師「|」$/g,""));showScreen("homeScreen");
};
document.querySelector("#endingSkipButton").onclick=()=>endingFinale(activeEndingId);
