...
function playDomesticEnding(entries){const cinematic=document.querySelector("#endingCinematic");let index=0;const next=()=>{if(index>=entries.length)return endingFinale(activeEndingId);const entry=entries[index++];cinematic.innerHTML=`<article class="ending-race-card"><small>GⅠ MEMORY ${String(index).padStart(2,"0")}</small><h2>${entry.raceName}</h2>${endingHorseMarkup(entry,index)}<b>${entry.horseName}</b><span>${entry.generation||1}代目</span></article>`;endingLater(next,2300)};next()}
function playOverseasEnding(entries){const cinematic=document.querySelector("#endingCinematic");let index=0;const next=()=>{if(index>=entries.length)return endingFinale(activeEndingId);const entry=entries[index++],order=entry.finishSnapshot?.order||[],pack=(order.length?order.slice(0,4):[{player:true,color:entry.horseColor},{color:"#75452f"},{color:"#b26a36"}]);cinematic.innerHTML=`<div class="ending-overseas-finish ${entry.finishSnapshot?.weather||""}"><div class="ending-finish-post"><i></i><b>GOAL</b></div><div class="ending-finish-pack">${pack.map((runner,i)=>`<div class="ending-finish-runner" style="--finish-gap:${i*16}px">${endingHorseMarkup(runner.player?entry:{horseColor:runner.horseColor||runner.color},i)}<em>${runner.id||i+1}</em></div>`).join("")}</div><article><small>WORLD GⅠ ${index} / ${entries.length}</small><h2>${entry.raceName}</h2><b>${entry.horseName}</b><span>${entry.finishSnapshot?.time||""}</span></article></div>`;endingLater(next,2600)};next()}
function playAllGradedEnding(entries,preview=false){const cinematic=document.querySelector("#endingCinematic"),minor=entries.filter(e=>e.grade!=="G1"),rollDuration=Math.min(24000,Math.max(14000,minor.length*190));cinematic.innerHTML=`<div class="ending-text-roll" style="--roll-duration:${rollDuration}ms"><div>${minor.map(e=>`<p><small>${e.grade==="G2"?"GⅡ":"GⅢ"}</small><b>${e.raceName}</b><span>${e.generation||"-"}代目 ${e.horseName}</span></p>`).join("")}</div></div>`;endingLater(()=>{const lineage=[...(game.retirementRecords||[]),{name:game.horseName,appearanceDNA:game.candidate?.appearanceDNA}].filter(h=>h.name),mock=ENDING_MOCK_HORSES.map((name,i)=>({name,horseColor:["#9b542e","#6f3f2b","#b46a35","#55423a"][i%4]})),horses=(preview||!lineage.length?mock:lineage).slice(-12);cinematic.innerHTML=`<div class="ending-parade"><h2>受け継がれた蹄跡</h2><div>${horses.map((horse,i)=>`<article style="--parade-delay:${i*180}ms">${endingHorseMarkup(horse,i)}<b>${horse.name}</b></article>`).join("")}</div></div>`;endingLater(()=>endingFinale(activeEndingId),6200)},rollDuration)}
