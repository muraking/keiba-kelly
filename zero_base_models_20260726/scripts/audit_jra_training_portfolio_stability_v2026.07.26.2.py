"""Audit temporal/venue robustness of JRA training-ranked longshot tickets.
...
def main():
    ...
    for kind in ("tan", "fuku", "umaren", "wide", "sanfuku", "santan1", "santan2", "santan3"):
        eligible = [
            r for r in rules
            if r["bet_type"] == kind
            and r["periods"]["2024"]["bets"] >= 100
            and r["periods"]["2025"]["bets"] >= 100
        ]
        selected_exploratory[kind] = max(
            eligible,
            key=lambda r: min(r["periods"]["2024"]["roi"], r["periods"]["2025"]["roi"]),
        )
        discovery = [
            r for r in eligible
            if r["periods"]["2024"]["max_share"] is not None
            and r["periods"]["2024"]["max_share"] <= .20
        ]
        selected_discovery[kind] = max(
            discovery,
            key=lambda r: (r["periods"]["2024"]["lcb90"], r["periods"]["2024"]["roi"]),
        )
    ...
    result = {
        "version": VERSION,
        "method": {
            "discovery_2024": "highest 2024 LCB90 with >=100 bets and max payout share <=20%; 2025 untouched",
            "exploratory_stable": "best minimum ROI across 2024 and 2025; retrospective screen",
        },
        "payout_coverage": coverage,
        "discovery_2024_audits": audits_discovery,
    ...
    for kind, audit in audits_discovery.items():
        a24, a25 = audit["annual"]["2024"], audit["annual"]["2025"]
        print("discovery", kind, round(a24["roi"], 1), round(a25["roi"], 1),
              "without-max", round(a24["roi_without_largest_return"], 1),
              round(a25["roi_without_largest_return"], 1))
